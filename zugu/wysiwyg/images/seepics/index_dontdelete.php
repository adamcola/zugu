			...
			while ( $collapse && ($item = readdir ( $dh )) )
				...
				if ( !preg_match ( "/^\./", $item ) )
					...
					if ( is_dir ( $posix_root."/".$item ) )	// If it is a directory, then we allow an AJAX call to open it.
						...
						echo "<a rel='facebox' class=\"a_class_file_link\" href=\"".htmlspecialchars($posix_root."/".$item)."\">";
						}
					...
					if ( $viable_link )
						{
						echo "</a>";
						echo "<a href='index_dontdelete.php?file=../$item&action=delete'
      title='delete the image'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<IMG SRC='delete.gif' title='delete this file?' border='0'></a>";  // alter the filename 
...
if (isset($_GET['action']) && isset($_GET['file'])) {
  if ($_GET['action'] == "delete") {
    unlink($_GET['file']);
	echo "<IMG SRC='recycle_orange_whiteoutline.gif' border='0'><meta http-equiv='refresh' content='3;url=index_dontdelete.php'>";
  }
} else {
echo ""; 
}
